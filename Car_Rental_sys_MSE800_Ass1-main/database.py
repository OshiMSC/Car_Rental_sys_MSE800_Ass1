...
def alter_completed_bookings_table():
    conn = create_connection()
    cursor = conn.cursor()

    # Add cost column if not exists
    cursor.execute("ALTER TABLE CompletedBookings ADD COLUMN cost REAL DEFAULT 0")
    # Add total_cost column if not exists
    cursor.execute("ALTER TABLE CompletedBookings ADD COLUMN total_cost REAL DEFAULT 0")
